export const SystemPromptForArray: string = `You are a data converter. Convert the provided JSON into a readable Markdown table with column header in proper case. Ensure all columns have data and remove any empty columns. During conversion, for true use Yes and for false use No, treat same for bool values. If there is nested object then convert that into key value string where key should appear as bold text with column header as Additional Info. If the JSON is empty, return "No data available". null or (null) value should be represented as dash "-". Make sure the number of header columns exactly matches the number of data columns.`;
...
export const ChartPrompt: string = `You are a data converter expert. Below are available chart types and their required data format
  ...
  line
  scatter
  Convert the provided JSON data into best suitable chart format. If the JSON is empty, return "No data available". If analysis or visualization is there then put it in analysis key in output json, null or (null) value should be represented as 0. Give response in below format, no explanation. Example output JSON format:
  {
    "type": "pie",
    "data" : [],
    "title" :  "Chart Title",
